...
#	endif
	...
	// draw sun disk
#	if defined(SKYBOX_SUNDISK_SIMPLE)
		half sunMie = calcSunSpot(u_SunDirection, normalize(-IN.worldViewVec.xyz));
#	else
		//float sunCos_Precious = dot(normalize(IN.worldViewVec.xyz), u_SunDirection);
		half sunMie = getMiePhase(sunCos, sunCos * sunCos);
#	endif

	sunColor = sunMie * 6 * _SunIntensity;
#endif

	return float4(skyDome.rgb * extinction + scatter + sunColor, 0.f);
}
ENDCG
