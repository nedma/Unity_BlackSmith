...
void _VolundTransferScatter(float3 _worldPos, out half4 coords1, out half4 coords2, out half4 coords3) {
	...
	// 1.calculates color
	// rayleigh/mie color only depends on angleY, which is (cam-->vertex).y
	...
	// 2. calculate scattering(density)
	...
	// 3.rayleighColor shifted to red by distance(cam-->vertex)
